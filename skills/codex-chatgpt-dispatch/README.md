...
`handoff.txt`；目标清楚的简单生图可以直接使用精炼提示词。每次附件任务会先静默
核对官方 Browser 能力边界和本机版本；官方仍不支持或证据不清时，Skill 会把页面和
档位准备好，给出便于复制选择的完整路径清单，由用户上传一次，再由 Browser 核对并
继续发送与回收。
...
- 发现明确恢复信号时只请求一次安全探测授权；授权前保持人工路径，探测不使用真实
  材料、不发送消息，失败后不连续重试。
- 进入人工路径时一次给全按目录分组的文件名、用途和绝对路径，暂停前使用
