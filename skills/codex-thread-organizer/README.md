...
`codex-thread-organizer` 用于维护 Codex 会话的可回溯性：给相关线程统一命名、在会话结束时生成短摘要和交接包、整理历史会话、提出索引和归档建议。
...
| `codex-thread-organizer:closeout` | 组合执行命名、五行摘要、必要交接包，并询问是否新开 |
...
- 如果线程工具暂不可用，skill 会降级为标题建议、收口摘要和交接包；连续主线编号需要等工具恢复后再补齐。
...
按命名规则收口当前会话；如有待完成或待确认事项，先给出交接包，再询问是否直接新开。
```
