...
**依赖与边界**：真实提交要求 Codex 桌面宿主提供可操作的内置 Browser；按当前公开契约选择 CUA 或旧 Browser 控制器，不绑定旧 Skill 名称。CUA 自动 AX 卡住时可按返回文档使用同一 IAB 的公开 DOM 路径，并依据实测耗时拆分调用、调整时限；该路径已完成“6 Pro”纯文本与单个小型 TXT 自动上传、单次发送及同页内容回收验证。可见控件的语义定位失败时核查实际 DOM，再选择当前契约支持的唯一定位；已补验“极高 → Pro”。其他附件能力单独核对，需人工上传时标记“人工辅助调度成功”。故障保留发送状态并交付材料；不静默降级、重发或换到外部浏览器。显式触发不等于发送授权。
