...
**依赖与边界**：真实提交要求 Codex 桌面宿主提供可操作的内置 Browser；按当前公开契约选择 CUA 或旧 Browser 控制器，不绑定旧 Skill 名称。附件能力按需核对，需人工上传时单独标记“人工辅助调度成功”。连接故障有界恢复后交付材料，不把离线验证当作自动链恢复。显式触发不等于发送授权。产品档位、模型、思考强度或发送状态不唯一时停止，不静默降级、重发或切换链路。
